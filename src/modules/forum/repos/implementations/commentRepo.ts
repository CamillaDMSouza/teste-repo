...
export class CommentRepo implements ICommentRepo {
  ...
  async getMemberCommentCount(memberId: MemberId): Promise<number> {
    const CommentModel = this.models.Comment;
    const detailsQuery = this.createBaseQuery();
    detailsQuery.where["member_id"] = memberId.id.toString();
    const commentCount = await CommentModel.count(detailsQuery);
    return commentCount;
  }

  async getNumberOfCommentsCreatedByDate(
    day: number,
    month: number,
    year: number
  ): Promise<number> {
    ...
    const result = await this.models.sequelize.query(
      `SELECT COUNT(*) FROM comment WHERE created_at = "${year}-${month}-${day}";`
    );
    const count = result[0][0]["COUNT(*)"];
    return count;
  }

  async getNumberOfCommentsOnDate(date: Date | string): Promise<number> {
    console.log(date);
    const result = await this.models.sequelize.query(
      `SELECT COUNT(*) AS NUM_COMMENTS FROM comment WHERE created_at = "${date}";`
    );
    const count = result[0][0]["NUM_COMMENTS"];
    return count;
  }

  async getNumberOfCommentsMembersOnDate(
    date: Date | string,
    memberId: string,
    ...
    console.log(date);
    console.log(
      `SELECT count(*) AS NUM_COMMENTS_MEMBERS FROM comment INNER JOIN member ON "${commentMemberId}"  = "${memberId}" WHERE date(created_at) = "${date}" group by member_id";`
    );
    const result = await this.models.sequelize.query(
      `SELECT count(*) AS NUM_COMMENTS_MEMBERS FROM comment INNER JOIN member ON comment.member_id = member.member_id WHERE created_at = "${date}" group by member_id";`
    );
    const count = result[0][0]["NUM_COMMENTS_MEMBERS"].length;
    return count;
  }
}
